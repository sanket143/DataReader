    ...
    for row in reader:
      ...
      try:
        ...
        if appName in apps:
          ...
          try:
            temp = dataWithDate; 
            duration = temp[appHour]["duration"];
            temp[appHour]["duration"] = addDuration(duration, appDuration);
...
for time in dataWithDate:
  timeArr = dataWithDate[time]["duration"].split(":");

  if int(timeArr[2]) > 30:
    timeArr[1] = int(timeArr[1]) + 1;

  timeArr[1] = int(timeArr[1]) * 100 / 60;
  timeDuration = str(timeArr[0]) + "." + str(timeArr[1]);
